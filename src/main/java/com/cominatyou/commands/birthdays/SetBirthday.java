...
public class SetBirthday {
    ...
    public static void set(MessageCreateEvent message, List<String> messageArgs) {
        ...
        if (birthdayStringExists) {
            MessageUtil.sendTextReply(message.getMessage(), "You already have a birthday set! If you want to change it, please use `h!birthday edit month-day` or use </birthday edit:1011153003853643816>.");
            return;
        }
        ...
        if (!birthday.matches("(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])") || birthday.length() == 4) {
            MessageUtil.sendTextReply(message.getMessage(), "Invalid birthday provided. Please provide your birthday in `mm-dd` format. (i.e., 06-21 is June 21)");
            return;
        }
        ...
        if (thirtyDayMonths.contains(month) && intDay > 30) {
            MessageUtil.sendTextReply(message.getMessage(), "That month only has 30 days! Double-check the date you provided.");
            return;
        }
        else if (thirtyOneDayMonths.contains(month) && intDay > 31) {
            MessageUtil.sendTextReply(message.getMessage(), "That month only has 31 days! Double-check the date you provided.");
            return;
        }
        else if (month.equals("02") && intDay > 29) { // Leap birthdays
            MessageUtil.sendTextReply(message.getMessage(), "February only has 29 days (at most)! Double-check the date you provided.");
            return;
        }
        ...
        if (intMonth == currentMonth && intDay == currentDay) {
            if (birthday.equals("02-29")) {
                MessageUtil.sendTextReply(message.getMessage(), "Happy Birthday! Your birthday has been set to February 29. Your birthday will be announced on March 1 on non-leap years.");
            }
            else {
                final String monthString = DateFormatSymbols.getInstance().getMonths()[intMonth - 1];
                MessageUtil.sendTextReply(message.getMessage(), String.format("Happy Birthday! Your birthday has been set to %s %d.", monthString, intDay));
            }
            ...
            MessageUtil.sendTextReply(message.getMessage(), "Success! Your birthday has been set to February 29. Your birthday will be announced on March 1 on non-leap years.");
        }
        else {
            final String monthString = DateFormatSymbols.getInstance().getMonths()[intMonth - 1];
