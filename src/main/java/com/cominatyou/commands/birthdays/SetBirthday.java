...
public class SetBirthday {
    ...
    public static void set(MessageCreateEvent message, List<String> messageArgs) {
        ...
        if (birthdayStringExists) {
            MessageUtil.sendTextReply(message, "You already have a birthday set! If you want to change it, please use `h!birthday edit month-day` or use </birthday edit:1011153003853643816>.");
            return;
        }
        ...
        if (thirtyDayMonths.contains(month) && intDay > 30) {
            MessageUtil.sendTextReply(message, "That month only has 30 days! Double-check the date you provided.");
            return;
        }
        else if (thirtyOneDayMonths.contains(month) && intDay > 31) {
            MessageUtil.sendTextReply(message, "That month only has 31 days! Double-check the date you provided.");
            return;
        }
        else if (month.equals("02") && intDay > 29) { // Leap birthdays
            MessageUtil.sendTextReply(message, "February only has 29 days (at most)! Double-check the date you provided.");
            return;
        }
        ...
        if (intMonth == currentMonth && intDay == currentDay) {
            if (birthday.equals("02-29")) {
                MessageUtil.sendTextReply(message, "Happy Birthday! Your birthday has been set to February 29. Your birthday will be announced on March 1 on non-leap years.");
            }
            else {
                final String monthString = DateFormatSymbols.getInstance().getMonths()[intMonth - 1];
                MessageUtil.sendTextReply(message, String.format("Happy Birthday! Your birthday has been set to %s %d.", monthString, intDay));
            }
            ...
            MessageUtil.sendTextReply(message, "Success! Your birthday has been set to February 29. Your birthday will be announced on March 1 on non-leap years.");
        }
        else {
            final String monthString = DateFormatSymbols.getInstance().getMonths()[intMonth - 1];
            MessageUtil.sendTextReply(message, String.format("Success! Your birthday has been set to %s %d.", monthString, intDay));
        }
    }
}
